#-------------------------------------------------
#
# Project created by QtCreator 2019-11-13T20:47:19
#
#-------------------------------------------------
...
macos {
    ...
    # 依赖模块
    LIBS += \
            -L$$PWD/thrid_party/ffmpeg/lib -lavformat.58 \
            -L$$PWD/thrid_party/ffmpeg/lib -lavcodec.58 \
            -L$$PWD/thrid_party/ffmpeg/lib -lavutil.56 \
            -L$$PWD/thrid_party/ffmpeg/lib -lswscale.5

    # mac bundle file
    APP_SCRCPY_SERVER.files = $$files($$PWD/thrid_party/scrcpy-server.jar)
    APP_SCRCPY_SERVER.path = Contents/MacOS
    QMAKE_BUNDLE_DATA += APP_SCRCPY_SERVER

    APP_ADB.files = $$files($$PWD/thrid_party/adb/mac/adb)
    APP_ADB.path = Contents/MacOS
    QMAKE_BUNDLE_DATA += APP_ADB

    APP_FFMPEG.files = $$files($$PWD/thrid_party/ffmpeg/lib/*.dylib)
